...
NSString * const BGPTUSBHubErrorDomain = @"BGPTUSBHubError";
...
BGPTUSBHubNotificationKey const BGPTUSBHubNotificationKeyDeviceID = @"DeviceID";
BGPTUSBHubNotificationKey const BGPTUSBHubNotificationKeyMessageType = @"MessageType";
BGPTUSBHubNotificationKey const BGPTUSBHubNotificationKeyProperties = @"Properties";
...
@implementation BGPTUSBHub


+ (BGPTUSBHub*)sharedHub {
  static BGPTUSBHub *gSharedHub;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    gSharedHub = [BGPTUSBHub new];
    [gSharedHub listenOnQueue:dispatch_get_main_queue() onStart:^(NSError *error) {
      if (error) {
        NSLog(@"BGPTUSBHub failed to initialize: %@", error);
      }
    } onEnd:nil];
  });
  return gSharedHub;
...
- (void)handleBroadcastPacket:(NSDictionary*)packet {
  NSString *messageType = [packet objectForKey:BGPTUSBHubNotificationKeyMessageType];
...
+ (NSDictionary*)packetDictionaryWithPacketType:(NSString*)messageType payload:(NSDictionary*)payload {
  ...
  if (bundleName) {
    packet = [NSDictionary dictionaryWithObjectsAndKeys:
              messageType, BGPTUSBHubNotificationKeyMessageType,
              bundleName, @"ProgName",
              bundleVersion, @"ClientVersionString",
              nil];
  } else {
    packet = [NSDictionary dictionaryWithObjectsAndKeys:messageType, BGPTUSBHubNotificationKeyMessageType, nil];
  }
...
- (BOOL)errorFromPlistResponse:(NSDictionary*)packet error:(NSError**)error {
  if (!*error) {
    ...
    if (!n) {
      *error = [NSError errorWithDomain:BGPTUSBHubErrorDomain code:(n ? n.integerValue : 0) userInfo:nil];
      return NO;
    }
    ...
    if (replyCode != 0) {
      ...
      *error = [NSError errorWithDomain:BGPTUSBHubErrorDomain code:replyCode userInfo:[NSDictionary dictionaryWithObject:errmessage forKey:NSLocalizedDescriptionKey]];
      return NO;
    }
  }
  return YES;
  ...
  dispatch_io_read(channel_, 0, sizeof(ref_upacket.size), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    off_t offset = sizeof(ref_upacket.size);
      ...
      if (upacket_len > kUsbmuxPacketMaxPayloadSize) {
        callback(
          [[NSError alloc] initWithDomain:BGPTUSBHubErrorDomain code:1 userInfo:@{
      ...
      if (upacket->protocol != USBMuxPacketProtocolPlist) {
        callback([[NSError alloc] initWithDomain:BGPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package protocol" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
      ...
      if (upacket->type != USBMuxPacketTypePlistPayload) {
        callback([[NSError alloc] initWithDomain:BGPTUSBHubErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:@"Unexpected package type" forKey:NSLocalizedDescriptionKey]], nil, upacket->tag);
        usbmux_packet_free(upacket);
        return;
      }
