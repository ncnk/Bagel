...
#import "BGPTUSBHub.h"
#import "BGPTDefines.h"

@class BGPTAddress;
@protocol BGPTChannelDelegate;

NS_ASSUME_NONNULL_BEGIN

PT_FINAL @interface BGPTChannel : NSObject

// Delegate
@property (strong, nullable) id<BGPTChannelDelegate> delegate;

// Communication protocol.
@property BGPTProtocol *protocol;
...
// Create a new channel using the shared BGPTProtocol for the current dispatch
// queue, with *delegate*.
+ (BGPTChannel *)channelWithDelegate:(nullable id<BGPTChannelDelegate>)delegate NS_SWIFT_UNAVAILABLE("");

// Initialize a new frame channel, configuring it to use the calling queue's
// protocol instance (as returned by [BGPTProtocol sharedProtocolForQueue:
//   dispatch_get_current_queue()])
- (id)init NS_SWIFT_UNAVAILABLE("");

//// Initialize a new frame channel with a specific protocol.
- (id)initWithProtocol:(BGPTProtocol *)protocol NS_SWIFT_UNAVAILABLE("");

// Initialize a new frame channel with a specific protocol and delegate.
- (id)initWithProtocol:(nullable BGPTProtocol *)protocol delegate:(nullable id<BGPTChannelDelegate>)delegate NS_SWIFT_NAME(init(protocol:delegate:));

// Connect to a TCP port on a device connected over USB
- (void)connectToPort:(int)port overUSBHub:(BGPTUSBHub *)usbHub deviceID:(NSNumber *)deviceID callback:(void(^)(NSError * _Nullable error))callback NS_SWIFT_NAME(connect(to:over:deviceID:callback:));
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError * _Nullable error, BGPTAddress *_Nullable address))callback NS_SWIFT_NAME(connect(to:IPv4Address:callback:));
...
PT_FINAL @interface BGPTAddress : NSObject
...
// Protocol for BGPTChannel delegates
@protocol BGPTChannelDelegate <NSObject>

@required
// Invoked when a new frame has arrived on a channel.
- (void)ioFrameChannel:(BGPTChannel *)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(nullable NSData *)payload NS_SWIFT_NAME(channel(_:didRecieveFrame:tag:payload:));

@optional
// Invoked to accept an incoming frame on a channel. Reply NO ignore the
// incoming frame. If not implemented by the delegate, all frames are accepted.
- (BOOL)ioFrameChannel:(BGPTChannel *)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize NS_SWIFT_NAME(channel(_:shouldAcceptFrame:tag:payloadSize:));;

// Invoked when the channel closed. If it closed because of an error, *error* is
// a non-nil NSError object.
- (void)ioFrameChannel:(BGPTChannel *)channel didEndWithError:(nullable NSError *)error NS_SWIFT_NAME(channelDidEnd(_:error:));

// For listening channels, this method is invoked when a new connection has been
// accepted.
- (void)ioFrameChannel:(BGPTChannel *)channel didAcceptConnection:(BGPTChannel *)otherChannel fromAddress:(BGPTAddress *)address NS_SWIFT_NAME(channel(_:didAcceptConnection:from:));
