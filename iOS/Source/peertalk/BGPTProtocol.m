...
static const uint32_t BGPTProtocolVersion1 = 1;

NSString * const BGPTProtocolErrorDomain = @"BGPTProtocolError";
...
static void _release_queue_local_protocol(void *objcobj) {
  if (objcobj) {
    BGPTProtocol *protocol = (__bridge_transfer id)objcobj;
...
@implementation BGPTProtocol


+ (BGPTProtocol*)sharedProtocolForQueue:(dispatch_queue_t)queue {
...
- (dispatch_data_t)createDispatchDataWithFrameOfType:(uint32_t)type frameTag:(uint32_t)frameTag payload:(dispatch_data_t)payload {
  ...
  frame->version = htonl(BGPTProtocolVersion1);
  ...
  dispatch_io_read(channel, 0, sizeof(PTFrame), queue_, ^(bool done, dispatch_data_t data, int error) {
    ...
    if (done) {
      ...
      if (!allData || dispatch_data_get_size(allData) < sizeof(PTFrame)) {
        callback([[NSError alloc] initWithDomain:BGPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
        return;
      }
      ...
      if (frame->version != BGPTProtocolVersion1) {
        callback([[NSError alloc] initWithDomain:BGPTProtocolErrorDomain code:0 userInfo:nil], 0, 0, 0);
      } else {
...
@implementation NSData (BGPTProtocol)
...
@implementation NSDictionary (BGPTProtocol)
