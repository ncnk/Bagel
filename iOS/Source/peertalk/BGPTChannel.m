...
#pragma mark -
@implementation BGPTChannel

@synthesize protocol = protocol_;

+ (BGPTChannel*)channelWithDelegate:(id<BGPTChannelDelegate>)delegate {
  return [[BGPTChannel alloc] initWithProtocol:nil delegate:delegate];
}


- (id)initWithProtocol:(BGPTProtocol *)protocol delegate:(id<BGPTChannelDelegate>)delegate {
  if (!(self = [super init])) return nil;
	protocol_ = protocol ? protocol : [BGPTProtocol sharedProtocolForQueue:dispatch_get_main_queue()];
  self.delegate = delegate;
  return self;
}


- (id)initWithProtocol:(BGPTProtocol *)protocol {
	return [self initWithProtocol:protocol delegate:nil];
}


- (id)init {
  return [self initWithProtocol:[BGPTProtocol sharedProtocolForQueue:dispatch_get_main_queue()] delegate:nil];
}
...
- (id<BGPTChannelDelegate>)delegate {
  return delegate_;
}


- (void)setDelegate:(id<BGPTChannelDelegate>)delegate {
  delegate_ = delegate;
  delegateFlags_ = 0;
  if (!delegate_) {
    return;
...
- (void)connectToPort:(int)port overUSBHub:(BGPTUSBHub*)usbHub deviceID:(NSNumber*)deviceID callback:(void(^)(NSError *error))callback {
...
- (void)connectToPort:(in_port_t)port IPv4Address:(in_addr_t)address callback:(void(^)(NSError *error, BGPTAddress *address))callback {
  assert(protocol_ != NULL);
  if (connState_ != kConnStateNone) {
  ...
  // Success
  NSError *err = nil;
  BGPTAddress *ptAddr = [[BGPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (BOOL)acceptIncomingConnection:(dispatch_fd_t)serverSocketFD {
  ...
  if (delegateFlags_ & kDelegateFlagImplements_ioFrameChannel_didAcceptConnection_fromAddress) {
    BGPTChannel *peerChannel = [[BGPTChannel alloc] initWithProtocol:protocol_ delegate:delegate_];
    __block BGPTChannel *localChannelRef = self;
    ...
    BGPTAddress *address = [[BGPTAddress alloc] initWithSockaddr:(struct sockaddr_storage*)&addr];
...
- (NSString*)description {
  ...
  return [NSString stringWithFormat:@"<BGPTChannel: %p (%@)%s%@>", self, (  connState_ == kConnStateConnecting ? @"connecting"
